...
* 모든 트랜스폼 적용 대상은 **중심축 센터**를 기준 위치로 사용하여 속성 적용
...
* 모든 플랫폼에서 일관적인 콘텐츠 
* 새로운 기기 오래된 기기에서 모두 정상 작동
* 웹 사이트 렌더링 난이도가 상대적으로 높음
* html 요소의 순서를 잘못 구성할 경우 반응형 구성 어려움
* 반응형웹을 성공하기 위해서 많은 경험 노력 숙련된 디자인 능력 필요
* 콘텐츠 수 용량 등에 따라 성능이 저하 될 수 있음


### 적응형 웹
* 사용자가 이용하는 디바이스에 맞춰 별도로 생성된 레이아웃을 불러오는 유형
* 원하는 기기와 플랫폼에 맞게 최적화된 크기 제공
* 광고 목적에 맞춰 콘테츠를 정확하게 노출
* 콘텐츠가 모든 기기에서 일관되지 않을 경우 형태 무너질 수 있음
* 새로운 기기에 대응하기 어려움
...
@media print {실행명령어}
@media screen and (규칙) and (규칙) {실행명령어}
@media (규칙) {실행명령어}
```

### 반응형웹 미디어쿼리 제작 준비
* 최소 최대 제한 너비 설정 : 각 디바이스 해상도 및 특정 조건에 따라 앱의 스타일(CSS) 변경 시 사용
    * `min-width` - 최소 제한 너비 설정
    * `max-width` - 최소 제한 너비 설정
* 모바일부터 시작하는 경우 : min-width 규칙 작은값->큰값 순서로 작성
* 데스크탑부터 시작하는 경우 : max-width 규칙 큰값->작은값 순서로 작성


### memo
* 가로크기 고정 `width: 1920px;`하는 것은 적응형 웹에서만
* `max-width: 1920px;` : 최대제한 너비 1920(사용자가 보는 디바이스 크기가 1920이하일 경우 상대적으로 자동 크기 조절)
* `min-width: 901px;` : 최소 제한 너비 901 (901~1920 사이값만 상대처리를 위한 최소제한값 설정)
* 반응형에서 요소 크기 (width)조절 시 반드시 % 또는 calc() 계산법으로 상대처리 해야 함
* 크기가 고정되는 광고 및 특정 영역만 필요시 최소한으로 고정 너비 적용
* `@media screen and (max-width: 900px) {}` : 반응형 규칙 (사용자가 이용하는 디바이스가 달라졌을 때 인식하는 값)
    * 기존 css 중 현재 규칙에 맞춰 달라지는 값만 수정
    * max-width: 900px; - 첫번째 규칙 max-width:1920에 의해 1920보다 작은 값은 min 규칙으로 막지 않는 한 모두 상대처리하기 때문에 반복해서 입력하지 않아도 됨
* 반응형에서 고정 영역 px허용+ `position:stiky`로 고정
* 처음부터 화면에 고정된 경우 - fixed
